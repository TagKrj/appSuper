...
namespace appSuper.Controller
{
    internal class TheThaoController
    {
        ...
        public bool CheckMa(string maSP)
        {
            using (SqlConnection conn = Database.GetConnection())
            {
                string query = "SELECT COUNT(*) FROM TheThao WHERE maSP = @maSP";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
        ...
        public static void ThemmoiTheThao(string maSP, string tenSP, string nhaCungCap, int soLuong, decimal giaNhap, decimal giaBan)
        {
            using (SqlConnection conn = Database.GetConnection())
            {
                string sql = "INSERT INTO TheThao (maSP, tenSP, nhaCungCap, soLuong, giaNhap, giaBan) VALUES (@maSP, @tenSP, @nhaCungCap, @soLuong, @giaNhap, @giaBan)";
